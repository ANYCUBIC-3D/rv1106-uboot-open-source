			  ...
			  bool verify)
{
	...
	if (part_get_info_by_name(dev_desc, part_name, &part) < 0) {
		...
		char slot[3];
		char bootslotname[32];
		rk_avb_get_current_slot(slot);
		sprintf(bootslotname,"boot%s",slot);
		if(part_get_info_by_name(dev_desc, bootslotname, &part) < 0){
			FIT_I("No %s partition\n", bootslotname);
			return NULL;
		}
		part_name = bootslotname;
		printf("enter boot %s\n",bootslotname);

		int sysnum;
		char sysname[32];
		char syscommand[128];
		disk_partition_t syspart;
		sprintf(sysname,"system%s",slot);
		sysnum = part_get_info_by_name(dev_desc, sysname, &syspart);
		if(sysnum < 0){
			FIT_I("No %s partition\n", sysname);
			return NULL;
		} else {
			sprintf(syscommand,"setenv sys_bootargs root=/dev/mmcblk0p%d",(sysnum));
			//setenv sys_bootargs ubi.mtd=%d root=ubi0:system rootfstype=ubifs rk_dma_heap_cma=66M
			//sprintf(syscommand,"setenv sys_bootargs ubi.mtd=%d root=ubi0:system rootfstype=ubifs rk_dma_heap_cma=66M",(sysnum - 1));
			run_command(syscommand, 0);
		}
#else
		FIT_I("No %s partition\n", part_name);
		return NULL;
