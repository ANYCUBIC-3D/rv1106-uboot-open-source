...
static void *do_boot_fit_storage(ulong *size)
{
	...
	void *i = fit_image_load_bootables(size);
	if(i == NULL){
		AvbABData ab_data;
		ops->ab_ops->read_ab_metadata(ops->ab_ops, &ab_data);
		if(ab_data.slots[1].priority > ab_data.slots[0].priority){
			ab_data.slots[1].tries_remaining++;
			ab_data.slots[1].priority = 14;
			ab_data.slots[0].priority = 15;
			if(ab_data.slots[0].tries_remaining){
				ab_data.slots[0].tries_remaining--;
			}
		} else {
			ab_data.slots[0].tries_remaining++;
			ab_data.slots[1].priority = 15;
			ab_data.slots[0].priority = 14;
			if(ab_data.slots[1].tries_remaining){
				ab_data.slots[1].tries_remaining--;
			}
		}
		ops->ab_ops->write_ab_metadata(ops->ab_ops, &ab_data);
		i = fit_image_load_bootables(size);
	}
	return i;
#else
