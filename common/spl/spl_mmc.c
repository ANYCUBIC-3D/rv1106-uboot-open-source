		       ...
		       struct spl_boot_device *bootdev)
{
	...
	case MMCSD_MODE_RAW:
		...
		if (!err){
			...
			spl_ab_data_read(mmc_get_blk_desc(mmc), &ab_data_spl, "misc");
			if(ab_data_spl.slots[1].priority > ab_data_spl.slots[0].priority){
				ab_data_spl.slots[1].tries_remaining++;
				ab_data_spl.slots[1].priority = 14;
				ab_data_spl.slots[0].priority = 15;
			} else {
				ab_data_spl.slots[0].tries_remaining++;
				ab_data_spl.slots[1].priority = 15;
				ab_data_spl.slots[0].priority = 14;
			}
			spl_ab_data_write(mmc_get_blk_desc(mmc), &ab_data_spl, "misc");
			err = mmc_load_image_raw_partition(spl_image, mmc,
				CONFIG_SYS_MMCSD_RAW_MODE_U_BOOT_PARTITION_NAME,
				CONFIG_SYS_MMCSD_RAW_MODE_U_BOOT_PARTITION);
			if (!err){
