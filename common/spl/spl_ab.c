...
int spl_ab_data_write(struct blk_desc *dev_desc, AvbABData *ab_data,
			     char *partition)
{
...
int spl_ab_data_read(struct blk_desc *dev_desc, AvbABData *ab_data,
			    char *partition)
{
	int ret;
	AvbABData serialized;
...
int spl_get_current_slot(struct blk_desc *dev_desc, char *partition, char *slot)
{
	...
	AvbABData ab_data_spl;
	spl_ab_data_read(dev_desc, &ab_data_spl, "misc");
	if (spl_slot_is_bootable(&ab_data_spl.slots[0]) &&
		spl_slot_is_bootable(&ab_data_spl.slots[1])) {
		if(ab_data_spl.slots[1].priority > ab_data_spl.slots[0].priority) {
			if(ab_data_spl.slots[1].tries_remaining > 0)
				ab_data_spl.slots[1].tries_remaining--;
			printf("slots[1]: tries_remaining:%d\n",ab_data_spl.slots[1].tries_remaining);
		} else {
			if(ab_data_spl.slots[0].tries_remaining > 0)
				ab_data_spl.slots[0].tries_remaining--;
			printf("slots[0]: tries_remaining:%d\n",ab_data_spl.slots[0].tries_remaining);
		}
	} else if (spl_slot_is_bootable(&ab_data_spl.slots[0])) {
		ab_data_spl.slots[0].tries_remaining--;
	} else if (spl_slot_is_bootable(&ab_data_spl.slots[1])) {
		ab_data_spl.slots[1].tries_remaining--;
	}

	spl_ab_data_write(dev_desc, &ab_data_spl, "misc");
	printf("slot:_b ab_data_spl.slots[1]: priority:%d tries_remaining:%d successful_boot:%d\n",ab_data_spl.slots[1].priority,ab_data_spl.slots[1].tries_remaining,ab_data_spl.slots[1].successful_boot);
	printf("slot:_a ab_data_spl.slots[0]: priority:%d tries_remaining:%d successful_boot:%d\n",ab_data_spl.slots[0].priority,ab_data_spl.slots[0].tries_remaining,ab_data_spl.slots[0].successful_boot);
#endif
