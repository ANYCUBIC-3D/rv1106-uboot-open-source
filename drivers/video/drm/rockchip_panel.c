					...
					struct rockchip_panel_cmds *cmds)
{
	...
	struct rockchip_cmd_desc *desc = &cmds->cmds[0];
	for (j = 0; j < 1; j++) 
	{
		for (i = 0; i < (cmds->cmd_cnt); i++) {
			desc = &cmds->cmds[i];
...
//test	 SPI dat
	...
	mdelay(50);
//	printf("del=100\n");
	rockchip_panel_write_spi_cmds(priv,0, 0x2a);
	rockchip_panel_write_spi_cmds(priv,1, 0);	
	rockchip_panel_write_spi_cmds(priv,1, 0);	
	rockchip_panel_write_spi_cmds(priv,1, 0);		
	rockchip_panel_write_spi_cmds(priv,1, 239&0xff);	
	
	rockchip_panel_write_spi_cmds(priv,0, 0x2b);
	rockchip_panel_write_spi_cmds(priv,1, 0);	
	rockchip_panel_write_spi_cmds(priv,1, 0);
	rockchip_panel_write_spi_cmds(priv,1, 319>>8);		
	rockchip_panel_write_spi_cmds(priv,1, 319&0xff);	
	
	rockchip_panel_write_spi_cmds(priv,0, 0x2c);
	mdelay(20);
	...
	for (j = 0; j < 160; j++) 
	{
		for (i = 0; i < 120; i++) 
		{
			rockchip_panel_write_spi_cmds(priv,1, 0xf8);
			rockchip_panel_write_spi_cmds(priv,1, 0x00);
		}
	...
	{	
		...
		for (j = 0; j < 320; j++) 
		{
			for (i = 0; i < 240; i++) 
			{
				rockchip_panel_write_spi_cmds(priv,1, 0x07);
				rockchip_panel_write_spi_cmds(priv,1, 0xE0);
			}	
		}				
	}
#endif
...
//		mdelay(plat->delay.reset);
	...
	printf("plat->delay.reset=: %d  +30ms\n", plat->delay.reset);	
	mdelay(30);	
	if (dm_gpio_is_valid(&priv->reset_gpio))
	dm_gpio_set_value(&priv->reset_gpio, 1);
...
static int rockchip_panel_probe(struct udevice *dev)
{
	...
	if (priv->cmd_type == CMD_TYPE_SPI) {
		...
		printf("---u-boot dm_gpio_set_value --------------\n");
		dm_gpio_set_value(&priv->spi_sdi_gpio, 1);
		dm_gpio_set_value(&priv->spi_scl_gpio, 1);
		dm_gpio_set_value(&priv->spi_cs_gpio, 1);
